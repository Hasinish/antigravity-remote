...
* **Activity Row Styling & Mappings Sync**: Fixed mismatched activity summaries in [helpers.ts](file:///D:/antigravity-remote/mobile-app/src/utils/helpers.ts) and [MessageFeed.tsx](file:///D:/antigravity-remote/mobile-app/src/components/MessageFeed.tsx). Implemented lookup matching using the closest initiating `PLANNER_RESPONSE`'s `tool_calls` array to dynamically fetch and display actual command texts, counts of files/folders in directory sweeps, correct absolute file paths, and render task-completion updates as centered dividers.
