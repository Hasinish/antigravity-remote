...
### 2026-06-16 (continued — Antigravity Accounts & Quotas Trackers)
* **Antigravity Active User Script**: Created [get-user.js](file:///D:/antigravity-remote/server/get-user.js) to query the active language server processes and return a clean, dedicated console display of the currently logged-in account name, email address, and plan tier.
* **Antigravity Model Quotas Script**: Created [get-quota.js](file:///D:/antigravity-remote/server/get-quota.js) to query the running Antigravity Language Server processes, dynamically extract CSRF tokens and active listening ports on Windows, query the Connect-RPC GetUserStatus endpoint, and format a clean, detailed model usage table showing name, email, plan subscription, remaining percentages, and reset timers for all active developer accounts.
* **Activity Row Styling & Mappings Sync**: Fixed mismatched activity summaries in [helpers.ts](file:///D:/antigravity-remote/mobile-app/src/utils/helpers.ts) and [MessageFeed.tsx](file:///D:/antigravity-remote/mobile-app/src/components/MessageFeed.tsx). Implemented lookup matching using the closest initiating `PLANNER_RESPONSE`'s `tool_calls` array to dynamically fetch and display actual command texts, counts of files/folders in directory sweeps, correct absolute file paths, and render task-completion updates as centered dividers. Set `alignSelf: 'flex-start'` on activity row containers in [styles.ts](file:///D:/antigravity-remote/mobile-app/src/styles.ts) so chevrons (`›`) sit compactly next to their labels instead of stretching to full-width.
* **Markdown Table Rendering Support**: Added custom block parsing and responsive layout styles to [MarkdownText.tsx](file:///D:/antigravity-remote/mobile-app/src/components/MarkdownText.tsx) to parse tables, render them with zebra striping, stretch them to 100% parent width on window resize, and fallback to horizontal scroll on narrow mobile screens (min-width constraint).
